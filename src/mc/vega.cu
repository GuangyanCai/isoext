...
struct cell_worker {
    ...
    __host__ __device__ int interior_test(int i, int flag13) {
        ...
        t = 0.5f * (v[3] * bt - v[2] * at + v[1] * dt - v[0] * ct) / t;   // -b/2a
        if (t > 0.0f && t < 1.0f) {
            at = v[0] + at * t;
            bt = v[1] + bt * t;
            ct = v[2] + ct * t;
    ...
    __host__ __device__ void find_case(unsigned int i) {
        ...
        switch (c >> 12) {   // find the MC33 case
            case 0:   // cases 1, 2, 5, 8, 9, 11 and 14
                ...
                break;
            case 1:   // case 3
                offset = ((m ? i : i ^ 0xFF) & face_test1(k >> 2))
                             ? 183 + 2 * k
                             : 159 + k;
                break;
            case 2:   // case 4
            ...
            case 4:   // case 7
                switch (face_tests(f, m ? i : i ^ 0xFF)) {
                    ...
                    case 1:   // 7.3
                        ...
                        break;
                    default:   // 7.4
                        offset = interior_test(k >> 1, 0) ? 1095 + 9 * k
                                                          : 1055 + 5 * k;
                }
                break;
            case 5:   // case 10
...
struct process_cube_op {
    ...
    process_cube_op(float3 *out, const uint *cell_indices,
                    const GridView &view, const unsigned short *luts,
                    const float level)
