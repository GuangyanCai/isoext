...
namespace mc::lorensen
{
    ...
    struct process_cube_op
    {
        ...
        process_cube_op(float3 *v, const float *grid, const int *edges,
                        const int *edge_table, const int *tri_table, const uint3 res,
                        const float level)
            : v(v),
              grid(grid),
              edges(edges),
        ...
        __host__ __device__ void operator()(
            thrust::tuple<uint8_t, uint32_t, uint32_t> args)
        {
            ...
            // Compute the location of each cube vertex. Assume each cube is a unit cube
            // for now.
            float3 p_pos[8];
    ...
    std::tuple<float *, uint32_t, int *, uint32_t> marching_cubes(
        float *const grid_ptr, const std::array<int64_t, 3> &grid_shape,
        const std::array<float, 6> &aabb, float level)
    {
        ...
        thrust::device_vector<int> edge_table_dv(edge_table, edge_table + 256);
        thrust::device_vector<int> tri_table_dv(tri_table, tri_table + 4096);
        ...
        thrust::transform(sorted_v_dv.begin(), sorted_v_dv.end(), sorted_v_dv.begin(),
