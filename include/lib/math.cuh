...
__host__ __device__ float3 interpolate(float iso_level, float val_0,
                                       float val_1, float3 pos_0,
                                       float3 pos_1)
{
    if (abs(iso_level - val_0) < 0.00001)
        return (pos_0);
    if (abs(iso_level - val_1) < 0.00001)
        return (pos_1);
    if (abs(val_0 - val_1) < 0.00001)
        return (pos_0);
    float mu = (iso_level - val_0) / (val_1 - val_0);
