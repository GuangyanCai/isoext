...
template <typename Framework, typename Device> struct MarchingCubes {
    // Input types
    using GridType =
        nb::ndarray<Framework, float, nb::ndim<3>, Device, nb::c_contig>;
    using AABBType = std::array<float, 6>;

    // Output types
    using Vertices =
        nb::ndarray<Framework, float, nb::shape<-1, 3>, Device, nb::c_contig>;
    using Faces =
